...
var condition = "CONDITION"; // random number between 0 and 4 (inclusive) - calculated below. 
...
function onDocLoaded() {
    ...
    // var game = baseurl + 'games/exampleGlobalExplanation.txt';
    ...
    if (condition == 2 || condition == 4) {
        if (condition == 4) {
            $('#global_explanation').show();
        }
        $('#globalModal').modal('show')
        // we only want to start the game after the user has closed the modal containing global explanation 
...
function nextGame() {
    ...
    if (maxScores[games[currentGame] - 1] == pathScores[0]) {
        // alert("matched");
        maxScoreGames++;
    }
    currentGame++;
    if (currentGame < 10) {
        alert("You will now start game " + (currentGame + 1) + " out of 10.");
        currentPlayer = 0;
        var display_game = document.querySelector('#game');
        display_game.textContent = "Game " + (currentGame + 1) + " out of 10. ";

        // loading new game
        var game = baseurl + 'games/game' + games[currentGame] + '.txt';
...
function storeProgress(action) {
    ...
    var stringCondition = "";
    if (condition == 0) {
        stringCondition = "noPathNoEx";
    }
    if (condition == 1) {
        stringCondition = "noEx";
    }
    if (condition == 2) {
        stringCondition = "localGlobalEx";
    }
    if (condition == 3) {
        stringCondition = "localEx";
    }
    if (condition == 4) {
...
function readScoreFile() {
    var rawFile = new XMLHttpRequest();
    rawFile.open("GET", "games/game_scores.txt", false);
    rawFile.onreadystatechange = function () {
        if (rawFile.readyState === 4) {
            if (rawFile.status === 200 || rawFile.status == 0) {
                var allText = rawFile.responseText;
...
function loadGame(instructions) {
    ...
    // console.log("Extra chips "+ extraChips);
...
function clickedCell(i, j) {
    ...
    if (cell.classList[3] == currentPlayer) {
        ...
        for (let index = 0; index < extraChipsPlayer.length; index++) {
            // (the part after && is to fix a bug where clicking a '1' will register as eating away the '1' from a '10')
        ...
        updateScore();
        paths[currentPlayer] = paths[currentPlayer].replace("  ", " ");
        ...
        if (addPath) {
            ...
            console.log(paths[currentPlayer]);
            paths[currentPlayer] = paths[currentPlayer].replace(i + " " + j + " " + 1, "");
            console.log(paths[currentPlayer]);
        }
        paths[currentPlayer] = paths[currentPlayer].replace("  ", " ");
...
function pathComplete(currentPlayer) {
    ...
    console.log("checking path complete");
    console.log(path);
...
function showPlayerPath(player) {
    ...
    // update local explanation indicators
